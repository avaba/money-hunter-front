...
import {TrackingPositionsRepository} from "@/repositories/tracking_positions_repository";
import { VuexTrackingPositionsStateGroupItemInterface } from "@/store/modules/trackingPositions";
...
export class TrackingPositionsService {
  ...
  private repo = new TrackingPositionsRepository();
  ...
  async getPosition(articul: string) {
    try {
      return (await this.authService.refreshWrapper(this.repo.getPosition.bind(this.repo, articul))).data.detail;
    } catch (e) {
      return [];
    }
  }

  async setPositions(articules: Array<string>) {
    try {
      return (await this.authService.refreshWrapper(this.repo.setPositions.bind(this.repo, {articules})));
    } catch (e) {
      return [];
    }
  }
