...
<script>
  ...
  export default {
    ...
    methods: {
      isActive(item) {
        if (item.system) {
          return this.$route.name === 'trackingPositions.group_list';
        }
      ...
      navigateTo(item) {
        if (item.system) {
          if (this.$route.name !== 'trackingPositions.group_list') {
            this.$router.push({name: 'trackingPositions.group_list'});
          }
        } else {
          if (!(this.$route.name === 'trackingPositions.group' && this.$route.params.name === item.label)) {
            this.$router.push({name: 'trackingPositions.group', params: {name: item.label}});
          }
        }
      },
      /**
